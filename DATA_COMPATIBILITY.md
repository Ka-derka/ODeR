...
ODeR 1.1 Alpha 1 adds optional presentation metadata to each profile without changing the profiles-state schema. Alpha 2 adds `odrlib` profiles, whose immutable package and extracted artwork live in the existing per-profile storage directory; no directory-cache schema is applied to them. Installed library profiles may retain extension badges such as `U1`. The `.odrlib` container remains format version 1 while compatible behaviour evolves through declared extensions: unknown optional extensions can be preserved, while unknown required extensions stop import safely. ODeR 1.0 preserves unknown profile fields when loading and saving its existing data, while `.oder` packages remain at format version 1 and profile schema version 1.
