...
Always publish stable tags as `vMAJOR.MINOR.PATCH` (for example, `v1.0.0`) and prerelease tags as canonical SemVer (for example, `v1.1.0-alpha.1` or `v1.1.0-rc.1`), not `0.21`, `v0.21`, or `v1.1.0-alpha1`. ODeR 0.19 and newer tolerate abbreviated older tags, but older installed versions require all three numeric parts. Canonical tags allow users to update directly without installing intermediate releases.
